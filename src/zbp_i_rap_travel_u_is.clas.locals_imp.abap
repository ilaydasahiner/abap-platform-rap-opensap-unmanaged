...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD update.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).
      ...
      legacy_entity_x-_intx = CORRESPONDING zsrap_travel_x_is( <entity> MAPPING FROM ENTITY ).
  ...
  METHOD read.
    " The read method is used to read data from the buffer rather than from the database.
    " This is for example needed, when doing updates, because we are using ETags
  ...
  METHOD cba_Booking.
  "we assume that new bookings cannot be created separately but can only in conjunction with a given travel instance.
  "The fact that new instances of the bookings can only be created for a specific travel instance is considered in the behavior definition.
...
CLASS lsc_ZI_RAP_TRAVEL_U_IS DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_RAP_TRAVEL_U_IS IMPLEMENTATION.
